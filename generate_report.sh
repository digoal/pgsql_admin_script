...
vi /etc/sysctl.conf
# add by digoal.zhou
kernel.shmmax = 135497418752     # 最大单个共享内存段大小
kernel.shmmni = 8192
...
net.ipv4.tcp_tw_timeout = 5   # 快速回收TIME_WAIT状态的TCP socket  https://en.wikipedia.org/wiki/Transmission_Control_Protocol
...
#  vm.nr_hugepages = 102352    #  建议shared buffer设置超过64GB时 使用大页，乘以/proc/meminfo Hugepagesize就是内存数量。
